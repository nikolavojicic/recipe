...
/ recipes and transformation and filtering of the results produced by the ~get~ method.
...
expression. One way to achieve this is to use ~of~ static factory method as a wrapper.
...
Supplier instead of the concrete Recipe type. That's why ~of~ method can also be used as
a converter to the Recipe type.
